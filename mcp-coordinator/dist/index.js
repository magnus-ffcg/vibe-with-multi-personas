...
async function getStorage(corpusName) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        case 'create_plan': {
            const storage = await getStorage(args.corpus_name);
            const plan = {
        ...
        case 'update_task': {
            const storage = await getStorage(args.corpus_name);
            await storage.updateTask(args.taskId, {
                status: args.status,
        ...
        case 'create_handoff': {
            ...
            const handoff = {
                ...
                notes: args.notes || '',
                blockers: Array.isArray(args.blockers) ? args.blockers : (args.blockers ? [args.blockers] : []),
        ...
        case 'set_current_persona': {
            const storage = await getStorage(args.corpus_name);
            await storage.setCurrentPersona(args.persona);
